...
def init_args():
    ...
    parser.add_argument('-s', '--model_save_dir', type=str,
                        help='Path to pre-trained weights to continue training')
...
def train_shadownet(dataset_dir, weights_path, char_dict_path, ord_map_dict_path, need_decode,  model_save_dir):
    """
    ...
    # Set tf summary
    tboard_save_dir = model_save_dir
    ...
    with sess.as_default():
        ...
        if weights_path is None or not os.path.exists(weights_path) or len(os.listdir(weights_path)) < 5:
            ...
            logger.info('Restore model from {}'.format(weights_path))
...
def train_shadownet_multi_gpu(dataset_dir, weights_path, char_dict_path, ord_map_dict_path, model_save_dir):
    """
